...
- `tensor-title` — load its `pattern.css` and `pattern.js`, then call `window.PtoTensorTitle.render(host, scene, options)`. The page supplies structured identity, shape, dtype, format, memory, owner, partition, state, provenance, step, axes, constraints, and status fields; the Pattern owns header DOM, byte formatting, and text escaping. Do not copy the header DOM/CSS, wrap it in a second card, add page-local colors, or use an iframe.
