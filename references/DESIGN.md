...
- `patterns/moe-routing/pattern.json`
- `patterns/moe-routing/pattern.html`
- `patterns/moe-routing/pattern.css`
- `patterns/moe-routing/pattern.js`
...
For MoE routing evidence, reuse pattern id `moe-routing`. The canonical source is the generated SVG renderer in `patterns/moe-routing/pattern.js`, extracted from `examples/openpangu-moe-routing`. It owns the 128-Rank single-layer layout, 256-expert Token Journey columns, Top-8 expert spokes, weighted-sum path, shared-expert track, overload state, active Rank slice, and anchored tooltips. Embedded workbenches must mount it with `showChrome:false` and drive view, Layer, Token, Rank scope, and theme through the controller instead of copying the standalone toolbar or SVG geometry.

For swimlane task bars, reuse pattern id `swimlane-task-bar`. The canonical source is the canvas renderer, task colormap helper, and hover tooltip helper in `patterns/swimlane-task/pattern.js`, aligned to `pypto-swimlane-perf-tool/js/swimlane.js` and the PMU swimlane `.sl-tooltip` interaction. Call `createTaskColormap` for semantic, stitch, engine, or subgraph colors instead of local hash palettes. Selection and emphasis preserve the resolved hue with only a small lightness adjustment, and each segment chooses light or dark label text from the final fill's contrast. Do not rebuild it with DOM/CSS or rewrite segment math, color hashing, colormap selection, color mixing, border alpha, contrast selection, label truncation, or hover tip behavior locally.
