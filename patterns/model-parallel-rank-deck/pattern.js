(function attachPtoModelParallelRankDeck(global){
  ...
  function mount(rootInput,options={}){
    ...
    if(groundGrid){groundGrid.position.y=-RANK_SIZE.y/2-.7;groundGrid.material.transparent=true;groundGrid.material.depthWrite=false;groundGrid.material.opacity=state.theme==='light'?.46:.2;groundGrid.renderOrder=-2;scene.add(groundGrid);}
    ...
    function addOverviewRankLabel(manifest){const rank=rankPositions.get(manifest.rank),asset=operatorLabelTexture(`R${manifest.rank}`,5),group=selectedGroup(),member=!group||group.ranks.includes(manifest.rank),material=new THREE.SpriteMaterial({map:asset.texture,transparent:true,depthTest:false,depthWrite:false,opacity:member?1:.16,toneMapped:false}),label=new THREE.Sprite(material);label.position.set(rank.x,rank.y+RANK_SIZE.y/2+1.05,rank.z);const scale=Math.max(3.2,asset.width*4.6);label.scale.set(scale,Math.max(.92,asset.height*4.6),1);label.renderOrder=21;label.userData.semanticId=`rank:${manifest.rank}/overview-label`;operatorLabelGroup.add(label);}
    ...
    function updateColors(){
      if(groundGrid){const gridColor=new THREE.Color(state.theme==='light'?'#d4d7db':'#3a3f45'),attribute=groundGrid.geometry.getAttribute('color');for(let index=0;index<attribute.count;index+=1)attribute.setXYZ(index,gridColor.r,gridColor.g,gridColor.b);attribute.needsUpdate=true;groundGrid.material.opacity=state.theme==='light'?.46:.2;}
