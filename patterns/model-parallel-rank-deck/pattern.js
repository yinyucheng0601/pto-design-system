(function attachPtoModelParallelRankDeck(global){
  ...
  function mount(rootInput,options={}){
    ...
    function rankColor(manifest){
      ...
      tempColor.set(value);if(!member)tempColor.lerp(new THREE.Color(backgroundColor()),state.theme==='light'?.985:.9);if(manifest.rank===state.selectedRank)tempColor.lerp(new THREE.Color('#ffffff'),.28);return tempColor.clone();
    }
    ...
    function updateColors(){
      ...
      rankMaterial.opacity=state.theme==='light'?.018:.065;
    ...
    function updateServerFrames(){
      ...
      for(let server=0;server<serverCount;server+=1){const ranks=Array.from({length:ranksPerServer},(_,index)=>server*ranksPerServer+index).filter(rank=>rankPositions.has(rank)),points=ranks.map(rank=>rankPositions.get(rank));if(!points.length)continue;const padX=RANK_SIZE.x*.78,padZ=RANK_SIZE.z*.72,minX=Math.min(...points.map(point=>point.x))-padX,maxX=Math.max(...points.map(point=>point.x))+padX,minZ=Math.min(...points.map(point=>point.z))-padZ,maxZ=Math.max(...points.map(point=>point.z))+padZ,color=server===active?(state.theme==='light'?0x4369ef:0x6f91ff):(state.theme==='light'?0xaeb4bd:0x64748b),positions=new Float32Array([minX,floorY,minZ,maxX,floorY,minZ,maxX,floorY,maxZ,minX,floorY,maxZ,minX,floorY,minZ]);
        const geometry=new THREE.BufferGeometry();geometry.setAttribute('position',new THREE.BufferAttribute(positions,3));const material=new THREE.LineBasicMaterial({color,transparent:true,opacity:server===active?.9:(state.theme==='light'?.38:.52),depthWrite:false}),line=new THREE.Line(geometry,material);line.frustumCulled=false;serverFrameGroup.add(line);
