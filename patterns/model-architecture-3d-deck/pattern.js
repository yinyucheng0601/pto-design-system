(function attachPtoModelArchitecture3dDeck(global){
  ...
  function nodeHtml(node,extra=''){
    return `<button type="button" class="pto-model-deck__node ${esc(extra)}" data-node="${esc(node.id)}" data-op="${esc(node.op||'linear')}" style="left:${node.x}px;top:${node.y}px;width:${node.w}px;height:${node.h}px">${esc(node.label)}</button>`;
  }
  ...
  function edgesHtml(nodes,edges,{width=720,height=1180}={}){
    const paths=edges.map(edge=>{
      ...
      return `<path data-kind="${esc(edge[2]||'activation')}" data-source="${esc(edge[0])}" data-target="${esc(edge[1])}" d="${d}"></path>`;
    }).join('');
    return `<svg class="pto-model-deck__edges" viewBox="0 0 ${width} ${height}" aria-hidden="true">${paths}</svg>`;
  }
  ...
  function clusterHtml(label,x,y,w,h,color){return `<div class="pto-model-deck__cluster" style="left:${x}px;top:${y}px;width:${w}px;height:${h}px;--cluster-color:${color}"><span>${esc(label)}</span></div>`;}
  function expertPoolHtml(nodes){const box={x:300,y:700,w:140,h:52};nodes.expert_pool={...box,id:'expert_pool',op:'moe'};return `<div class="pto-model-deck__experts is-collapsed" data-node="expert_pool" data-op="moe" role="button" tabindex="0" aria-label="Expert Pool · 72 experts" style="left:${box.x}px;top:${box.y}px;width:${box.w}px;height:${box.h}px">Expert Pool</div>`;}
  function layerHtml(layer,config){
    ...
    parts.push(`<div class="pto-model-deck__layer-label">L${layer}<span>PP${stage} · L${stageRange[0]}-${stageRange[1]} · ${dense?'Dense':'MoE'} · ${attn}${blockPost?' · block-post':''}</span></div>`);
    ...
    return `<section class="pto-model-deck__layer${layer===config.frontLayer?' is-front-layer':''}" data-layer="${layer}" data-stage="${stage}" data-stage-role="${first?'first':'repeat'}" style="--deck-opacity:${(1-.38*(layer/45)).toFixed(3)};transform:translate3d(0,0,${-layer*config.depthGap}px)"><div class="pto-model-deck__graph">${edgesHtml(nodes,edges)}${parts.join('')}</div></section>`;
  }
  function staticHtml(kind,config){
    ...
    if(input){
      ...
      return `<section class="pto-model-deck__static pto-model-deck__static--input" style="transform:translate3d(0,0,${config.depthGap*1.55}px)"><div class="pto-model-deck__static-title">Model input · source checked</div>${edgesHtml(nodes,edges,{height:178})}${parts.join('')}</section>`;
    }
    ...
    return `<section class="pto-model-deck__static pto-model-deck__static--output" style="transform:translate3d(0,0,${-(config.layerCount+.5)*config.depthGap}px)"><div class="pto-model-deck__static-title">Main output + MTP tail · source checked</div>${edgesHtml(nodes,edges,{height:526})}${parts.join('')}</section>`;
  }
  function shellHtml(config){
    return `<div class="pto-model-deck__title">${esc(config.label)}<span>CSS 3D · ${config.layerCount} layers</span></div>
      ...
      <div class="pto-model-deck__viewport">
        <div class="pto-model-deck__scene">${staticHtml('input',config)}${Array.from({length:config.layerCount},(_,layer)=>layerHtml(layer,config)).join('')}${staticHtml('output',config)}</div>
        <svg class="pto-model-deck__pp-wireframe" aria-hidden="true"></svg>
        <div class="pto-model-deck__pp-labels" aria-hidden="true"></div>
  ...
  function mount(rootInput,options={}){
    ...
    root.classList.add('pto-model-deck'); root.innerHTML=shellHtml(config);
    ...
    function pivot(){return{x:0,y:0,z:-(config.layerCount-1)*config.depthGap/2};}
    ...
    function setView(view){
      ...
      syncButtons();fit();options.onViewChange?.(state.view,api);return api;
    }
    function setParallelMode(mode){state.parallelMode=normalizeParallelMode(mode);syncButtons();scheduleOverlay();options.onParallelModeChange?.(state.parallelMode,api);return api;}
    ...
    function selectNode(nodeId,layer){
      ...
      const node=scope?.querySelector(`[data-node="${CSS.escape(String(nodeId))}"]`);node?.classList.add('is-selected');node?.closest('.pto-model-deck__layer')?.classList.add('is-selected');state.selected=node?{nodeId,layer}:null;options.onNodeSelect?.(state.selected,node,api);return node;
    }
    function ppGeometry(){
      const x=376,y=606;
    ...
    function renderPp(){
      ...
      const metrics={perspective,originX:width/2,originY:height/2,sceneLeft:width/2+state.panX,sceneTop:height/2+state.panY};
      ...
      ppGeometry().forEach(item=>{const points=item.corners.map(p=>project(p,matrix,metrics)),a=points.slice(0,4),b=points.slice(4),az=a.reduce((s,p)=>s+p.z,0)/4,bz=b.reduce((s,p)=>s+p.z,0)/4,near=az>=bz?0:4,far=near?0:4;
        ...
        const nearPoints=near?a:b,top=nearPoints.slice().sort((p,q)=>p.y-q.y).slice(0,2),label=document.createElement('span');label.className='pto-model-deck__pp-label';label.textContent=`PP${item.stage} · L${item.lo}–L${item.hi}`;ppLabels.appendChild(label);
    ...
    function renderAnnotations(){
      annotations.innerHTML='';if(state.view!=='front'||state.parallelMode==='none')return;
      const card=root.querySelector('.pto-model-deck__layer.is-front-layer')||root.querySelector('.pto-model-deck__layer'),base=annotations.getBoundingClientRect();
      config.annotations.filter(item=>item.kind===state.parallelMode).forEach(item=>{const node=card?.querySelector(`[data-node="${CSS.escape(item.nodeId)}"]`);if(!node)return;const rect=node.getBoundingClientRect();if(!rect.width)return;const badge=document.createElement('div');badge.className='pto-model-deck__annotation';badge.dataset.kind=item.kind;badge.innerHTML=`<b>${esc(item.kind.toUpperCase())}</b>${esc(item.label)}`;annotations.appendChild(badge);badge.style.left=`${rect.right-base.left}px`;badge.style.top=`${rect.top+rect.height/2-base.top}px`;badge.style.display='flex';});
    }
    function renderSideLabels(){
      sideLabels.innerHTML='';if(state.view!=='right')return;
      const card=root.querySelector('.pto-model-deck__layer.is-front-layer')||root.querySelector('.pto-model-deck__layer'),base=sideLabels.getBoundingClientRect();if(!card)return;
      const rows=config.sideRows.map(row=>{const nodes=row.ids.map(id=>card.querySelector(`[data-node="${CSS.escape(id)}"]`)).filter(Boolean);if(!nodes.length)return null;const rects=nodes.map(node=>node.getBoundingClientRect()),y=rects.reduce((sum,r)=>sum+r.top+r.height/2,0)/rects.length-base.top,color=getComputedStyle(nodes[0]).backgroundColor,left=Math.min(...rects.map(r=>r.left))-base.left,right=Math.max(...rects.map(r=>r.right))-base.left;return{row,y,color,left,right};}).filter(Boolean);
      rows.forEach(item=>{const rule=document.createElement('button');rule.type='button';rule.className='pto-model-deck__side-rule';rule.style.cssText=`left:${item.left}px;top:${item.y}px;width:${Math.max(8,item.right-item.left)}px;--side-color:${item.color}`;rule.addEventListener('click',()=>selectNode(item.row.ids[0]));const label=document.createElement('div');label.className='pto-model-deck__side-label';label.textContent=item.row.label;label.style.cssText=`left:${Math.min(base.width-160,item.right+14)}px;top:${item.y}px;--side-color:${item.color}`;sideLabels.append(rule,label);});
    }
    function renderOverlays(){raf=0;if(destroyed)return;renderPp();renderAnnotations();renderSideLabels();}
    ...
    viewport.addEventListener('pointerdown',pointerDown);viewport.addEventListener('pointermove',pointerMove);viewport.addEventListener('pointerup',pointerUp);viewport.addEventListener('pointercancel',pointerUp);viewport.addEventListener('wheel',wheel,{passive:false});scene.addEventListener('click',nodeClick);
    const resizeObserver=global.ResizeObserver?new ResizeObserver(resize):null;resizeObserver?.observe(root);
    const api={root,state,config,setView,setParallelMode,setTheme,setZoom,fit,setFrontLayer,selectNode,destroy(){destroyed=true;cancelAnimationFrame(raf);resizeObserver?.disconnect();viewport.removeEventListener('pointerdown',pointerDown);viewport.removeEventListener('pointermove',pointerMove);viewport.removeEventListener('pointerup',pointerUp);viewport.removeEventListener('pointercancel',pointerUp);viewport.removeEventListener('wheel',wheel);scene.removeEventListener('click',nodeClick);}};
    setTheme(state.theme);Object.assign(state,VIEW_POSES[state.view]);requestAnimationFrame(fit);return api;
  }
